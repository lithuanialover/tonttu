        ...
        <div class="flex auth-form show-form">
            <form method="post" action="{{ route('lates.update', $late->id) }}" enctype="multipart/form-data">
                ...
                <div class="auth-input flex">
                    ...
                    <input type="hidden" name="student_id" value="{{ $late->student_id }}" >
                </div>
                <!-- 遅刻日 -->
                <div class="auth-input flex">
                    <label for="day" value="day">遅刻日</label>
                    <input id="day"  class="input-css" type="date" name="day" required value = "{{ $late->day }}">
                </div>
                <!-- 到着時間 -->
                <div class="auth-input flex">
                    <label for="time" value="time">到着時間</label>
                    <input id="day"  class="input-css" type="time" name="time" required value = "{{ $late->time }}">
                </div>
                <!-- 送迎者 -->
                <div class="auth-input flex">
                    <label for="parent" value="parent">送迎者</label>
                    <input id="parent-input"  class="input-css" type="text" name="parent" required value = "{{ $late->parent }}">
                </div>
